...
function Encrypt(bufData) {
	...
	var CRC = GenerateCrc(encrypted.slice(0,offset), session.CRCSeed) & 0xffff;
	var crcSeed = session.CRCSeed;
    encrypted.writeUInt16BE(CRC, offset);
	//console.log("CRC Seed: " + crcSeed + " , CRC: " + CRC);
...
DecodeSWGPacket[0x6D2A6413] = function(data) {
	data.off = 0;
	data.off += 1;
	var ret = {type: "ChatSystemMessage"};
