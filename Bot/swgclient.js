...
module.exports.sendTell = function(player, message) {
    if (!module.exports.isConnected) return;
    //onsole.log("sending tell to: " + player + ": " + message);
...
handlePacket["ClientPermissions"] = function(packet) {
    ...
    setTimeout(() => {
		...
		send("ChatCreateRoom", {RoomPath: `SWG.${server.ServerName}.${server.SpatialRoom}`})
...
handlePacket["ChatRoomList"] = function(packet) {
    ...
    for (var roomID in packet.Rooms) {
        ...
        } else if(room.RoomPath.endsWith(server.SpatialRoom)) {
			server.SpatialRoomID = room.RoomID;
			send("ChatEnterRoomById", {RoomID: room.RoomID});
		}
    }
}
handlePacket["ChatOnEnteredRoom"] = function(packet) {
    //console.log(JSON.stringify(packet, null, 2));
    if ((packet.RoomID == server.ChatRoomID || packet.RoomID == server.SpatialRoomID )&& packet.PlayerName == server.Character) {
...
handlePacket["ChatRoomMessage"] = function(packet) {
	...
	else if(packet.RoomID == server.SpatialRoomID && packet.CharacterName != server.Character.toLowerCase())
		module.exports.recvSpatialChat(packet.Message, packet.CharacterName);
}
...
handlePacket["ChatSystemMessage"] = function(packet) {
	module.exports.recvSystemMsg(packet.Message);
}
