...
int LuaCreatureObject::getHeight(lua_State* L) {
	Locker locker(realObject);
	float height = realObject->getHeight();
...
int LuaCreatureObject::getShockWounds(lua_State* L) {
	Locker locker(realObject);
	int amount = realObject->getShockWounds();
...
int LuaCreatureObject::doCombatAnimation(lua_State* L) {
	String animString = lua_tostring(L, -1);
	CreatureObject* target = (CreatureObject*)lua_touserdata(L, -2);

	Locker locker(realObject);

	if (target != nullptr)
		BorEffect::PlayCombatAnim(realObject, target, true, animString);	
