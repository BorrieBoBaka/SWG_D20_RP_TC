...
public:
	...
	int doQueueCommand(CreatureObject* creature, const uint64& target, const UnicodeString& arguments) const {
					...
					if (object->isVehicleObject()) {
						...
						if (group->hasMember(vehicleOwner) && speeder->hasRidingCreature() && speeder->hasOpenSeat()) {
							Core::getTaskManager()->executeTask([=]() {
								Locker vlocker(speeder);
								speeder->slotPassenger(creature);	
							}, "slotVehiclePlayerPassenger");
						}
					} else {
						Creature* speeder = cast<Creature*>(vehicle);
						if (group->hasMember(vehicleOwner) && speeder->hasRidingCreature() && speeder->hasOpenSeat()) {
							Core::getTaskManager()->executeTask([=]() {
								Locker clocker(speeder);
								speeder->slotPassenger(creature);	
							}, "slotCreaturePlayerPassenger");
						}
					}
				}
			}			 
		...
		if (creature->getParent() != nullptr || (vehicle->getParent() != nullptr && !vehicle->isPassengerSeat()))
			return GENERALERROR;
