...
int CraftingSessionImplementation::startSession() {
	...
	/*
	if(craftingTool->getToolType() == CraftingTool::JEDI) {
		//Populate with Lightsaber Schematics
		String schemTemplate = "object/draft_schematic/roleplay/weapon/lightsaber/shared_sword_saber_qu_rahn.iff";
		DraftSchematic* schematic = SchematicMap::instance()->get(schemTemplate.hashCode());
		crafterGhost->addRewardedSchematic(schematic, SchematicList::MISSION, 1, true);
		currentSchematicList.add(schematic);
