...
void TangibleObjectImplementation::setDefender(SceneObject* defender, bool overrideNullCheck, bool overrideSelfCheck) {
	
	if(!overrideSelfCheck) {
	...
	if (defenderList.size() == 0) {
		addDefender(defender, overrideNullCheck, overrideSelfCheck);
		return;
	}
...
void TangibleObjectImplementation::addDefender(SceneObject* defender, bool overrideNullCheck, bool overrideSelfCheck) {
...
void TangibleObjectImplementation::fillAttributeList(AttributeListMessage* alm, CreatureObject* object) {
	...
	String dmCrafter = getStoredString("dm_creator");
	if(!dmCrafter.isEmpty()) {
		alm->insertAttribute("crafter", dmCrafter);
	} else if (!craftersName.isEmpty()) {
		alm->insertAttribute("crafter", craftersName);
	}

	String dmLastMod = getStoredString("dm_last_modified");
	if(!dmLastMod.isEmpty()) {
		alm->insertAttribute("last_dm", dmLastMod);
	}
