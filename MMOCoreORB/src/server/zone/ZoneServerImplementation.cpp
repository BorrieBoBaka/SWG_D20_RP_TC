...
void ZoneServerImplementation::initialize() {
	...
	roleplayCombatAnimManager = new RoleplayCombatAnimManager(_this.getReferenceUnsafeStaticCast());
	roleplayCombatAnimManager->loadLuaConfig();
...
Reference<SceneObject*> ZoneServerImplementation::createObject(uint32 templateCRC, int persistenceLevel, uint64 oid) {
	...
	try {
		...
		if (obj == nullptr) {
			error() << "could not create object! Template CRC = 0x" << hex << templateCRC;
		}
